 ...
 * and are Self-Shipped & Prepaid.
 *
 * Flow:
 * 1. Fetch all draft invoices from Zoho Billing
 * 2. Find matching orders in the DB (selecting invoiceItems to verify COD status)
 * 3. Filter for orders that are Self-Shipped AND Prepaid
 * 4. Convert those draft invoices to Open and record a banktransfer payment
 *
 * Usage:  bun run scripts/find_draft_delivered.ts
 */
...
function getISTDateString(): string {
    const date = new Date();
    // Offset for IST is +5:30 (330 minutes)
...
async function recordPaymentForInvoice(
    ...
    const payload = {
        ...
        amount: amount,
        date: getISTDateString(), // Fixed to IST timezone
...
async function main() {
    ...
    const matchingOrders = await Order.find({
        ...
        // Select includes invoiceItems to accurately check for COD
        .select('orderId zohoInvoiceId waybill shipments selfShipped status selfShipmentStatus paymentMode customerDetails invoiceItems')
        .lean() as any[];
    ...
    // 5. Check delivery status & payment mode for each order
    ...
    for (let i = 0; i < matchingOrders.length; i += BATCH_SIZE) {
        ...
        await Promise.all(
            batch.map(async (order: any) => {
                ...
                // --- 1. STRICT PREPAID CHECK ---
                const hasCodCharge = order.invoiceItems?.some((item: any) => 
                    item.name?.toUpperCase().includes('COD') || 
                    item.description?.toUpperCase().includes('CASH ON DELIVERY')
                ) || false;
                
                const currentPaymentMode = order.paymentMode || (hasCodCharge ? 'COD' : 'Prepaid');
                const isPrepaid = currentPaymentMode.toUpperCase() !== 'COD' && !hasCodCharge;

                // --- 2. STRICT SELF-SHIPPED CHECK ---
                const isSelfShipped =
                    order.selfShipped === true ||
                    order.status === 'SELF_SHIPPED' ||
                    (order.shipments && order.shipments.some((s: any) => 
                        s.vendor === 'SELF' || s.deliveryPartner === 'SELF'
                    ));

                // --- EARLY EXIT ---
                if (!isSelfShipped || !isPrepaid) {
                    process.stdout.write(` ⏭ Skipping ${order.orderId} — (Self-Shipped: ${isSelfShipped}, Prepaid: ${isPrepaid})\n`);
                    return; 
                }

                // Push valid orders to be marked as Paid
                ...
                process.stdout.write(`  ✔ ${order.orderId} — Self-shipped & Prepaid (Queued for Payment)\n`);
            })
        );
    ...
    if (deliveredOrders.length === 0) {
        console.log('\n✅ No orders found that are DRAFT on Zoho AND (Self-Shipped + Prepaid).\n');
    } else {
        console.log(
            `\n⚠️  Found ${deliveredOrders.length} order(s) to process for payment:\n`
        );

        for (const o of deliveredOrders) {
            console.log(`  • ${o.orderId}  |  Zoho: ${o.zohoInvoiceId}  |  ${o.customerName}`);
        }
        ...
        for (const o of deliveredOrders) {
            ...
            process.stdout.write(` ⏳ Updating ${o.orderId} (Invoice: ${invoiceData.invoice_number})...`);

            // Step 1: Convert Draft → Open
            const isOpen = await convertInvoiceToOpen(token, invoiceData.invoice_id);

            if (isOpen) {
                // Step 2: Record payment with banktransfer
                const isPaid = await recordPaymentForInvoice(
                    token,
                    invoiceData.invoice_id,
                    invoiceData.customer_id,
                    invoiceData.total,
                    'Prepaid' // Forced prepaid because COD is already filtered out
                );

                if (isPaid) {
                    process.stdout.write(` ✅ Marked as PAID (banktransfer)!\n`);
                    updatedCount++;
                }
            }
    ...
    console.log('\n' + '─'.repeat(60));
    console.log(`Total DB matches checked: ${checkedCount} | Eligible for Payment: ${deliveredOrders.length}`);
