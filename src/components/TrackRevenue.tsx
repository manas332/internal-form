...
import OrderDetailsExpanded, {
  OrderData,
  InvoiceItem,
  formatCurrency,
  getOrderLineSum,
  getOrderTotal,
} from "./OrderDetailsExpanded";
...
export default function TrackRevenue() {
  ...
  const [trackingDataMap, setTrackingDataMap] = useState<
    Record<string, TrackingShipmentData[]>
  >({});
    ...
    try {
      ...
      if (data.Error) {
        if (
          ...
          throw new Error(
            "Shipment created, but not yet scanned by Delhivery.",
          );
        }
    ...
    <div className="max-w-4xl mx-auto w-full">
      ...
      {!loading &&
        ...
        data.map((sp, idx) => {
          ...
          const barWidth =
            maxRevenue > 0 ? (sp.totalRevenue / maxRevenue) * 100 : 0;
                    ...
                    >
                      ...
                      >
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            <span
                              className={`text-gray-400 text-xs transition-transform ${expandedOrders.has(order._id) ? "rotate-90 text-indigo-500" : ""}`}
                            >
                              ▶
                            </span>
                      ...
                      {expandedTracking.has(order._id) && (
                        <div className="animate-fadeIn mt-2 mb-4 p-4 border border-indigo-100 dark:border-indigo-900/50 bg-indigo-50/50 dark:bg-indigo-900/10 rounded-xl relative">
                          ...
                          {trackingError[order._id] && (
                            <div className="text-sm text-red-500 dark:text-red-400">
                              ⚠️ {trackingError[order._id]}
                            </div>
                          )}

                          {!trackingLoading[order._id] &&
                            trackingDataMap[order._id] &&
                            trackingDataMap[order._id]!.length > 0 &&
                            (() => {
                                  ...
                                  {shipments.map((trackingData, tIdx) => (
                                    ...
                                    >
                                      ...
                                      <div className="space-y-4 relative before:absolute before:inset-0 before:ml-[11px] before:-translate-x-px before:h-full before:w-0.5 before:bg-indigo-200 dark:before:bg-indigo-900/50">
                                        {trackingData.Shipment.Scans?.map(
                                          (scanItem: any, idx: number) => {
                                            const scan =
                                              "ScanDetail" in scanItem
                                                ? scanItem.ScanDetail
                                                : scanItem;
                                            return (
                                              <div
